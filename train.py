...
# dataset = keypoint_dataset(data_dir="./keypoint_dataset", transform=custom_transform)
...
# train_set, val_set = random_split(dataset, [train_size, val_size+test_size])
...
# def imshow(img, l):
#     img_new = img[0].permute(1,2,0)
#     img_new = img_new*(torch.tensor(config.STD)*config.MAX_PIX_VAL) + (torch.tensor(config.MEAN)*config.MAX_PIX_VAL)
#     img_new = img_new.int()
#     npimg = img_new.numpy()
#     plt.imshow(npimg)
#     plt.plot(l[0].numpy()[0::2], l[0].numpy()[1::2], "go")
#     for ind, (x, y) in enumerate(zip(l[0].numpy()[0::2], l[0].numpy()[1::2])):
