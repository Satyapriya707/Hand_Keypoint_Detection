...
from config import minVal, maxVal, height_sensitivity_factor, width_sensitivity_factor, mouse_speed, \
  click_wait_time, click_sleep_time, minPixel, depth_multiply_factor, \
    hand_detection_confidence, hand_tracking_confidence, frame_to_skip, \
      frame_skip_pixel, scr_height_factor, scr_width_factor
from mouse_utils import check_activity, do_activity, if_pause, if_skip_frame, if_move
...
while capture.isOpened():
    ...
    ######################## logic ###########################
    if results.multi_hand_landmarks and not if_pause(lm, landmarks[4].x, landmarks[4].y, landmarks[8].x, \
      landmarks[8].y, minVal, maxVal) and if_move(lm, maxVal, landmarks[8].z, landmarks[12].z, \
        landmarks[16].z, depth_multiply_factor):
      if if_skip_frame(last_frame_landmark[0], last_frame_landmark[1], landmarks[8].x, \
        landmarks[8].y, width_sensitivity_factor, height_sensitivity_factor, scr_width, scr_height, frame_skip_pixel):
        # time.sleep(0.4)
        skip_frame_count += 1   
        if start_flag:
          start_flag = False
        ...
        if frame_to_skip < skip_frame_count:
          skip_frame_count = 1
        continue
    if not start_flag:
      if results.multi_hand_landmarks and not if_pause(lm, landmarks[4].x, landmarks[4].y, landmarks[8].x, \
        ...
        if act == "left_click":
          ...
          if time.time() - l_cl_start >= click_wait_time:
            do_activity(act, landmarks[8].x, landmarks[8].y, last_frame_landmark[0], last_frame_landmark[1], \
              width_sensitivity_factor, height_sensitivity_factor, scr_width, scr_height, mouse_speed, click_sleep_time)
            l_cl_flag = True
          ...
          if not l_cl_flag:
            l_cl_flag = True
          do_activity(act, landmarks[8].x, landmarks[8].y, last_frame_landmark[0], last_frame_landmark[1], \
            width_sensitivity_factor, height_sensitivity_factor, scr_width, scr_height, mouse_speed, click_sleep_time, minPixel)
        elif act == "drag":
          if not r_cl_flag:
            r_cl_flag = True
          if not l_cl_flag:
